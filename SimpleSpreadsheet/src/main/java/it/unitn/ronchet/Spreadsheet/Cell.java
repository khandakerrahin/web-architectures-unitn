...
public class Cell {
    ...
    @Override
    public Cell clone(){
        Cell clone=new Cell(this.getId(),this.formula);
    ...
    Set<Cell> recursiveEvaluateCell() {
        ...
        for (Cell x: engine.dependance.get(getId())) {
    ...
    private static void createAndAddCell(String id, String formula){
        ...
        boolean isOK=c.checkCircularDependencies(c.getId());
        if (isOK) {
            System.out.println("it.unitn.ronchet.Spreadsheet.Cell "+c.getId()+" has no circular references");
            ssEngine.addCell(c);
        } else {
            System.out.println("it.unitn.ronchet.Spreadsheet.Cell "+c.getId()+" has circular references - not added");
        }
        System.out.println("====================================");
    }
